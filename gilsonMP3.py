...
# ----------------------------------------------------------------------
class APump():
	def __init__(self, com_port = 'COM3', verbose = True, parameters = False):
		...
		# TRUE direction: CW FALSE direction: CCW
		# TRUE keypad: <-, division FALSE keypad: <-, subtraction
		self.read_length = 40
	...
	def confirmRemoteControl(self):
		response = self.sendImmediate(self.pump_ID, '?') # requesting mode status
		if response == 'R':
	...
	# ------------------------------------------------------------------
	# Enable Remote Control (is remote is set to true)
	# Note: these are buffered commands (executed one at a time)
	# ------------------------------------------------------------------
	def enableRemoteControl(self, remote):
		if remote:
			self.sendBuffered(self.pump_ID, 'SR') #setting to remote mode (buffered command)
		else:
			self.sendBuffered(self.pump_ID, 'SK') #setting to keypad mode (buffered command)
	...
	# Status options: stopped, flowing
	# Direction options: not running, forward, reverse
	# Control options: keypad, remote
	# ------------------------------------------------------------------
	def getStatus(self):
		message = self.readDisplay()
		print(message)
		
		# getting status of direction
	...
	# ------------------------------------------------------------------
	# Master Reset
	# Note: done so as an immediate command
	# ------------------------------------------------------------------
	def masterReset(self):
		return self.sendImmediate(self.pump_ID, '$') 

	# ------------------------------------------------------------------
	# Read Display (on keypad)
	# Note: done so as an immediate command
	# ------------------------------------------------------------------
	...
	def setFlowDirection(self, forward):
		if self.flip_flow_direction: # if true, it will flip whatever direction it is
			if forward:
				self.sendBuffered(self.pump_ID, 'K<') #CCW, backwards
			else:
				self.sendBuffered(self.pump_ID, 'K>') #CW, forwards
		else:
			if forward:
				self.sendBuffered(self.pump_ID, 'K>') #CW, forwards
			else:
	...
	# ------------------------------------------------------------------
	# Set Pump Speed: between 0 and 48 as a buffered command
	# ------------------------------------------------------------------
	...
	# ------------------------------------------------------------------
	# Start Pump Flow
	# MOST IMPORTANT COMMAND FOR CONTROLLING PUMP
	# ------------------------------------------------------------------
	...
	# ------------------------------------------------------------------
	def stopFlow(self):
		self.sendBuffered(self.pump_ID, 'KH') #KH is an immediate stop
		return True
...
if (__name__ == '__main__'):
	...
	if pump.confirmRemoteControl():
		response = input('Start flow? Y/N ') #if statement entered when 
		if response == 'Y' or response == 'y':
			# can comment out the lines above if no need for confirmation
			pump.startFlow(speed = 15, direction = 'Backward') # 15 mL/min on the display
			print('Flow started')
	
			pause_time = 10 # time in seconds for flow to continue
